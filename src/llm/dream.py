    ...
    async def _run(self, api_call: ApiCaller) -> None:
        ...
        try:
            ...
            system = [{"type": "text", "text": (
                ...
                "2. 个人情报归位：如果群备忘录中包含关于某个人的信息"
                "（性格、偏好、背景、身份、观点、与他人的关系），"
                "将这些信息迁移到该用户的备忘录（user_QQ号），从群备忘录中删除。"
                "群备忘录的「成员」区域只保留 @QQ号(昵称) 和群内角色（管理员/成员），"
                "不写个人特征。判断标准：信息跟着人走（换群也成立）→ user_；"
                "只在本群语境下有意义 → group_。\n"
                "3. 跨文件交叉验证：读取相关联的备忘录（如某用户提到的群、群里提到的用户），"
                "检查信息是否一致，修正矛盾或过时内容。\n"
                "4. 其他问题（悬空引用、超长等）：自主修复。\n"
                f"{sticker_section}"
                f"{template_section}"
